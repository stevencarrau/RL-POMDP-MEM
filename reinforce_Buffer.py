...
class VApproximationWithNN(Baseline):
    def __init__(self,
        ...
        # TODO: implement here
        self.state_dims = state_dims
        self.obs_dims = state_dims/2
        self.buffer_size = 2
        self.alpha = alpha
        beta1 = 0.9
        beta2 = 0.999
        self.X = tf.placeholder("float", [None, self.obs_dims*self.buffer_size])
    ...
    def __call__(self,s) -> float:
        # TODO: implement this method
        pred = self.sess.run(self.yhat,feed_dict={self.X:s.reshape(-1,self.obs_dims*self.buffer_size)})
        return pred[0][0]

    def update(self,s,G):
        # TODO: implement this method
        self.sess.run(self.train_net,feed_dict={self.X:s.reshape(-1,self.obs_dims*self.buffer_size),self.Y:np.array([[G]])})
        return None
