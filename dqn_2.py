...
class DQNetwork:
    def __init__(self, state_size=4, action_size=2, hidden_size=10, learning_rate=0.01, name='DQNetwork'):
            ...
            for episode in tqdm(range(1, train_episodes)):
                ...
                while t < max_steps:
                    ...
                    if explore_prob > np.random.rand():
                        ...
                        Qs = sess.run(QN.output, feed_dict={QN.inputs_: state.reshape((1, *state.shape))})
                        action = np.argmax(Qs)
                    ...
                    # Train Network
                    Q_target = sess.run(QN.output, feed_dict={QN.inputs_: state_primes})
