...
from reinforce_Estimate import REINFORCE as RF_Buffer, PiApproximationWithNN as Pi_Buffer, ReplayMemory
...
def test_reinforce_Estimate(with_baseline, mem_size, runs):
	env = gym.make("CartPole-v0")
	gamma = 1.
	alpha = 3e-4
...
if __name__ == "__main__":
	...
	for e_i in range(3):
		...
		while not done:
			...
			action = drqn_policies[best_pol](z, memory)
			# Take action
			state_prime, reward, done, _ = env.step(action)
			z_prime = np.matmul(obs_mask, state_prime)
			# capture frame
